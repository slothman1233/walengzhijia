...
/**
 * 获得新闻内容，通过列表集合返回
 * @param {number} newsType 新闻类型，当为0的时候默认按照时间排序返回最新的10条新闻记录
...
class News {
    // 获得新闻内容，通过列表集合返回
    // newsType 新闻类型，当为0的时候默认按照时间排序返回最新的10条新闻记录
    ...
    async GetNewsList(params: GetNewsListModel) {
        return await http.get<ResIndustryTypeModelListReturnModel>(`${config.apiPath}api/Product/GetNewsList`, { params, headers: { 'Content-Type': 'application/json' } })
    }

    // 添加新闻
    // @CacheInterceptor('Product_GetProductIndustry', CacheTime.Min30)
    async AddNews(params: NewsInfoModel) {
        return await http.post<bodyModel<boolean>>(`${config.apiPath}api/Product/AddNews`, { params, headers: { 'Content-Type': 'application/json' } })
    }

    // 修改新闻快讯内容
    // @CacheInterceptor('Product_GetProductIndustry', CacheTime.Min30)
    async UpdateNews(params: NewsInfoModel) {
        return await http.post<bodyModel<boolean>>(`${config.apiPath}api/Product/UpdateNews`, { params, headers: { 'Content-Type': 'application/json' } })
    }

    // 删除新闻信息
    // @CacheInterceptor('Product_GetProductIndustry', CacheTime.Min30)
    async DeleteNews(params: NewsInfoModel) {
        return await http.post<bodyModel<boolean>>(`${config.apiPath}api/Product/DeleteNews`, { params, headers: { 'Content-Type': 'application/json' } })
    }
