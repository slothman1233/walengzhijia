 ...
 * @param {number} timetick 默认传0，有值的时候会根据当前时间戳往后自动获取10条新闻记录信息
 */
export const GetNewsByCompanyId = async (companyId: number = 0, newsType: number = 0, timetick: number = 0): Promise<bodyModel<ResNewsModel[]>> => await http.get<any>(`/api/news/GetNewsByCompanyId`, { params: { companyId, timetick, newsType } })
