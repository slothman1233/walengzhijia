...
let fs = require('fs')
let less = require('gulp-less') // less 编译
...
let cleanCss = require('gulp-clean-css') //压缩cssr
let del = require('del') //删除文件1
let rename = require('gulp-rename') //  重命名
let logger = require('gulp-logger')
let rev = require('@stl/wl-gulp-rev')
let revCollector = require('@stl/wl-gulp-rev-collector')
...
let gulpif = require('gulp-if') //判断
let karmaServer = require('karma').Server
let build = require('./node/gulp/build')
let config = require('./node/gulp/config')
...
let minimist = require('minimist') //命令行参数解析引擎
let compress = minimist(process.argv.slice(2)).compress ? JSON.parse(minimist(process.argv.slice(2)).compress) : false
let iswatch = minimist(process.argv.slice(2)).watch ? JSON.parse(minimist(process.argv.slice(2)).watch) : false
let filepath = paths.jspages
//js的chagne会触发多次
let jsLodingOver = true
...
//entries函数
let entries = function (dev) {
    let jsDir = path.resolve(dev)
    let entryFiles = glob.sync(jsDir)
    let map = {}
    for (let i = 0; i < entryFiles.length; i++) {
        let filePath = entryFiles[i]
        let filename = filePath.substring(filePath.lastIndexOf('/dist/') + 6, filePath.lastIndexOf('.'))
